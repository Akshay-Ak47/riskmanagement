...
pub fn create_risk_versions_table(
    conn: &Connection
) {
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS risk_versions (
            ...
            FOREIGN KEY(parent_issue_key)
            REFERENCES risks(issue_key)
            ON DELETE CASCADE
    ...
    .expect(
        "Failed to create risk_versions table"
    );

    println!(
        "Risk Versions table ready for use "
    );
}